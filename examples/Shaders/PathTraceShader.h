...
class PathTracer : public IHitShader
{
  public:
    std::unique_ptr<IHitShader> Clone() const override { return std::make_unique<PathTracer>(*this); }

    ShaderOutput Shade(const std::uint64_t                  id,
                       const HitShaderInput&                shaderInput,
                       const std::vector<IShaderResource*>& shaderResource,
                       RayGeneratorOutput&                  newRays) const
    {
