...
namespace
{
    ...
    void refit(BoundingBox& aabb, const std::vector<IIntersectable*>& objects, double looseness)
    {
    ...
    bool isObjectInLeftSplit(const Vector3D& splittingPlane, int currentSplittingPlane, IIntersectable* const& object)
    {
    ...
    void sortObjectsIntoBuckets(const std::vector<IIntersectable*>& objects,
    ...
    void split(std::vector<IIntersectable*>&       leftChild,
               std::vector<IIntersectable*>&       rightChild,
    ...
    void refitLeaf(DBVHNode& node, IIntersectable& leaf)
    {
    ...
    {
        SAHs[SwapLeftLeftToRight] = node.boundingBox.getSA() + rotations.leftLeft.sa + rotations.right.sa +
                                    rotations.leftRight.sa + rotations.swapLeftLeftToRight.getSA();
        SAHs[SwapLeftRightToRight] = node.boundingBox.getSA() + rotations.leftRight.sa + rotations.right.sa +
    ...
    void splitEven(const std::vector<IIntersectable*>& objects,
                   std::vector<IIntersectable*>&       left,
                   std::vector<IIntersectable*>&       right)
    {
    ...
    bool bestSplittingPlaneExists(int bestSplittingPlane) { return bestSplittingPlane != -1; }

    void switchBoxOrder(std::vector<IIntersectable*>& leftObjects, std::vector<IIntersectable*>& rightObjects)
    {
    ...
    void createLeftLeaf(DBVHNode& node, const std::vector<IIntersectable*>& leftObjects)
    {
        node.leftLeaf     = leftObjects.at(0);
        node.maxDepthLeft = 1;
    }

    void createRightLeaf(DBVHNode& node, const std::vector<IIntersectable*>& rightObjects)
    {
        node.rightLeaf     = rightObjects.at(0);
        node.maxDepthRight = 1;
    }

    inline void intersectLeaf(const Ray& ray, IntersectionInfo& intersectionInfo, IIntersectable& leaf, bool& hit)
    {
    ...
    {
        double distance = 0;
        if (rayBoxIntersection((child->boundingBox.minCorner), (child->boundingBox.maxCorner), ray, distance))
        {
            stack[stackPointer++] = child;
        }
    }

    inline void intersectLeaf(std::vector<IntersectionInfo>& intersectionInfo, const Ray& ray, IIntersectable& leaf)
    {
        IntersectionInfo info{false, std::numeric_limits<double>::max()};
        leaf.IntersectFirst(info, ray);
        if (info.hit)
    ...
    inline void getChildrenIntersections(const Ray&        ray,
                                         const DBVHNode&   node,
                                         ...
                                         bool&             hit)
    {
        if (isNodeRight(node))
        {
            ...
            right           = rayBoxIntersection(
                rightChild->boundingBox.minCorner, rightChild->boundingBox.maxCorner, ray, distanceRight);
        }
        else
        {
        ...
        if (isNodeLeft(node))
        {
            ...
            left           = rayBoxIntersection(
                leftChild->boundingBox.minCorner, leftChild->boundingBox.maxCorner, ray, distanceLeft);
        }
        else
        {
            // TODO request leaf if missing
    ...
    inline void pushIntersectionsOnStack(const DBVHNode&     node,
                                         double              distanceRight,
                                         double              distanceLeft,
                                         bool                right,
                                         bool                left,
    ...
    inline bool getChildrenIntersection(IntersectionInfo& intersectionInfo,
                                        const Ray&        ray,
    ...
    bool traverseAny(DBVHNode& root, IntersectionInfo& intersectionInfo, const Ray& ray)
    {
        if (root.maxDepthRight >= 64 || root.maxDepthLeft >= 64)
        {
            std::vector<const DBVHNode*> stack(root.maxDepthRight > root.maxDepthLeft ? root.maxDepthRight + 1
                                                                                      : root.maxDepthLeft + 1);
            stack[0] = &root;
            return processTraversalStack(intersectionInfo, ray, stack.data());
        }
    ...
    inline void getChildrenIntersection(std::vector<IntersectionInfo>& intersectionInfo,
                                        const Ray&                     ray,
                                        const DBVHNode**               stack,
                                        uint64_t&                      stackPointer)
    {
    ...
    bool addFirstAndOnlyElement(DBVHNode& root, const std::vector<IIntersectable*>& objects)
    {
...
void DBVHv2::setSurfaceAreaLeftLeft(DBVHNode& node, const Rotations& rotations, const double* SAHs)
{
    ...
    node.leftChild->surfaceArea = rotations.right.sa + rotations.leftRight.sa + rotations.swapLeftLeftToRight.getSA();
}
...
void DBVHv2::setSurfaceAreaLeftRight(DBVHNode& node, const Rotations& rotations, const double* SAHs)
{
    ...
    node.leftChild->surfaceArea = rotations.right.sa + rotations.leftLeft.sa + rotations.swapLeftRightToRight.getSA();
}
...
void DBVHv2::setSurfaceAreaRightLeft(DBVHNode& node, const Rotations& rotations, const double* SAHs)
{
    ...
    node.rightChild->surfaceArea = rotations.left.sa + rotations.rightRight.sa + rotations.swapRightLeftToLeft.getSA();
}
...
void DBVHv2::setSurfaceAreaRightRight(DBVHNode& node, const Rotations& rotations, const double* SAHs)
{
    ...
    node.rightChild->surfaceArea = rotations.left.sa + rotations.rightLeft.sa + rotations.swapRightRightToLeft.getSA();
}
...
bool DBVHv2::removeRightLeftGrandChild(DBVHNode& currentNode, DBVHNode& child, const IIntersectable& object)
{
...
bool DBVHv2::removeRightRightGrandChild(DBVHNode& currentNode, DBVHNode& child, const IIntersectable& object)
{
...
bool DBVHv2::removeLeftLeftGrandChild(DBVHNode& currentNode, DBVHNode& child, const IIntersectable& object)
{
    auto grandChild = child.leftLeaf;
    if (*grandChild != object)
        return false;
...
bool DBVHv2::removeLeftRightGrandChild(DBVHNode& currentNode, DBVHNode& child, const IIntersectable& object)
{
    auto grandChild = child.rightLeaf;
    if (*grandChild != object)
        return false;
...
bool DBVHv2::removeRightLeaf(DBVHNode& currentNode, const IIntersectable& object)
{
...
bool DBVHv2::removeLeftLeaf(DBVHNode& currentNode, const IIntersectable& object)
{
...
void DBVHv2::remove(DBVHNode& currentNode, const IIntersectable& object)
{
...
void DBVHv2::createNewParentForLeftLeafs(DBVHNode& node, const std::vector<IIntersectable*>& leftObjects)
{
    auto buffer           = node.leftLeaf;
...
bool DBVHv2::insertSingleObjectLeft(DBVHNode& node, const std::vector<IIntersectable*>& leftObjects)
{
    if (isEmptyLeft(node))
    {
...
bool DBVHv2::passObjectsToLeftChild(DBVHNode& node, const std::vector<IIntersectable*>& leftObjects)
{
...
void DBVHv2::createNewParentForRightLeafs(DBVHNode& node, const std::vector<IIntersectable*>& rightObjects)
{
    auto buffer           = node.rightLeaf;
    auto parent           = tree.newNode();
    parent->leftLeaf      = buffer;
...
bool DBVHv2::insertSingleObjectRight(DBVHNode& node, const std::vector<IIntersectable*>& rightObjects)
{
...
bool DBVHv2::passObjectsToRightChild(DBVHNode& node, const std::vector<IIntersectable*>& rightObjects)
{
...
double DBVHv2::computeSAHWithNewParent(const DBVHNode&    node,
...
double DBVHv2::evaluateBucket(const DBVHNode&                     node,
                              const std::vector<IIntersectable*>& objects,
                              const Vector3D&                     splittingPlane,
                              SplitOperation&                     newParent)
{
...
std::vector<double> DBVHv2::evaluateSplittingPlanes(const DBVHNode&                     node,
                                                    const std::vector<IIntersectable*>& objects,
...
void DBVHv2::add(DBVHNode& currentNode, const std::vector<IIntersectable*>& objects, uint8_t depth)
{
...
bool DBVHv2::addOntoSingleElement(const std::vector<IIntersectable*>& objects)
{
    if (!isLastElement(*root))
        return false;
    std::vector<IIntersectable*> newObjects{objects}; // TODO: make more efficient
...
DBVHv2::DBVHv2(const std::vector<IIntersectable*>& objects) : tree(FlatTree(4096)), root(tree.newNode())
{
    addObjects(objects);
}

void DBVHv2::addObjects(const std::vector<IIntersectable*>& objects)
{
...
void DBVHv2::removeObjects(const std::vector<IIntersectable*>& objects)
{
    for (const auto& object : objects)
    {
        if (isEmpty(*root))
...
bool DBVHv2::IntersectAll(std::vector<IntersectionInfo>& intersectionInfo, const Ray& ray) const
{
    if (isEmpty(*root))
        return false;
    if (isLastElement(*root))
...
std::unique_ptr<IIntersectable> DBVHv2::Clone() const
{
    std::vector<IIntersectable*> objects;
    for (const auto& array : tree.flatTree)
    {
        for (std::uint64_t i = 0; i < array.size; ++i)
        {
            const auto& node = array.at(i);
            if (isLeafLeft(node))
            {
                objects.push_back(node.leftLeaf);
            }
            if (isLeafRight(node))
            {
                objects.push_back(node.rightLeaf);
            }
        }
    }
    return std::make_unique<DBVHv2>(objects);
}

BoundingBox DBVHv2::GetBoundaries() const { return root->boundingBox; }

double DBVHv2::GetSurfaceArea() const { return root->surfaceArea; }
